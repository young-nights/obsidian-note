...
### 5.3 `Unknown USB filter 'hrdevmon'` 警告

执行 `usbipd bind` 时可能看到：
...
`hrdevmon` 是 Windows 上注册的**第三方 USB 过滤驱动**（filter driver），usbipd 启动时扫描到它，担心它会抢占 USB 设备控制权，因此提前提醒。**这是低风险提示，不是错误。**

**常见来源**：

| 来源类型 | 示例 |
|----------|------|
| 企业管控 / DLP 防泄漏软件 | USB 设备审计、U 盘管控 |
| 厂商设备监控工具 | HP / Huawei 等设备管理套件 |
| 安全软件 | 终端防护、上网行为管理 |
| 工控 / 硬件配套工具 | 烧录器、调试器自带驱动 |

**处理方式**：先直接 bind，根据结果决定是否强制：

```powershell
# 先正常尝试
usbipd bind --busid 1-3

# 若 bind 报错，或 attach 后 WSL 内看不到设备，改用强制模式
...
`--force` 让 usbipd 优先于 `hrdevmon` 接管该设备。副作用仅限于**该 USB 设备在 Windows 侧可能失去原监控软件的功能**，对系统和其他设备无影响。如需恢复：
...
| V1.1 | 2026-09-16 | 新增 5.3 节：Unknown USB filter 'hrdevmon' 警告说明、来源分类、--force 强制绑定流程与恢复方法 |
