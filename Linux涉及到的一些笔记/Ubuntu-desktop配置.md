...
# <font size=4>Git安装</font>

sudo apt install git

# <font size=4>vim安装</font>
...
在进行嵌入式Linux开发时，需要保证 Ubuntu 与 Windows 和 开发板处于同一个网段中。因此，这里选择使用桥接的网络模式。

```bash

# <font size=4>在Windows的命令提示符中查看宿主机的IP地址，网关IP以及子网掩码等信息</font>
...
设置完成后验证ip地址是否与宿主机桥接成功

```bash

# <font size=4>在Windows的命令提示符中ping通Ubuntu的网络</font>
...
# <font size=4>自动确认并安装ssh服务</font>

sudo apt install openssh-server -y

# <font size=4>检查状态(应该显示active（running）)</font>

sudo systemctl status ssh

# <font size=4>如果没启动，手动启动</font>

sudo systemctl start ssh

# <font size=4>建议设置成开机启动</font>

sudo systemctl enable ssh

# <font size=4>如果没开ufw可以跳过这一步，一般桌面版默认没开启，服务器版有些会开启</font>

# <font size=4>允许ssh通过ufw防火墙</font>

sudo ufw allow ssh

# <font size=4>重启建议防火墙</font>

sudo ufw reload

# <font size=4>查询Ubuntu的IP地址</font>
...
Ubuntu 默认自带的编译器是基于X86框架的 GCC 编译器，现在需要编译ARM框架的代码，X86下的GCC编译器就不适用，因此需要安装一个运行在X86框架上的GCC编译器。Linaro官网链接：[Linaro下载链接](developer.arm.com/downloads/-/arm-gnu-toolchain-downloads)
...
解释：
- arm-  ： ARMv7及更早的32位构架(Cortex-A7/A9)
- aarch64-  ： ARMv8-A的64位构架，小端字节序(Cortex-A53/A72)
- aarch64_be- ：ARM64位构架，大端字节序
- none-  ： 无特定芯片生产厂商，通用版本
- gnueabihf：GNU EABI硬浮点版本，使用FPU硬件加速浮点运算
...
# <font size=4>1.检查目标目录是否存在</font>

ls -a ~/linux/tool

# <font size=4>1.1 如果不存在，创建目标目录 </font>

sudo mkdir ~/linux/tool

# <font size=4>1.2 再次检查目录是否存在</font>

ls -a ~/linux/tool 

# <font size=4>2. 刚下载玩的包在Downloads文件夹中，移动到目标目录</font>
