...
CodeObservatory（代码演化观察站）是一个独立的跨平台桌面应用。它不是 IDE 插件，不是 Obsidian 附属，而是一个完整独立的桌面工具。

### 核心宗旨

**解决使用智能体（AI Agent）修改项目文件时的黑盒环境问题。**

当 AI Agent 自动修改代码时，开发者面临三个痛点：
1. **不可见**：不知道 Agent 改了哪些文件
2. **不可追溯**：无法追踪每个文件的变更历史
3. **不可关联**：不清楚文件之间的依赖和影响关系

CodeObservatory 提供三个核心能力：
- **文件变更可追踪**：自动记录每次文件变更（创建/修改/删除），时间线可视化
- **关系图谱版本追踪**：宇宙星河图谱展示文件树结构，一目了然项目全貌
- **文件链接管理**：通过图结构展示文件间依赖关系，快速定位影响范围
